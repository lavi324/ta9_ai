...
`groupadd -r wildfly`
`useradd -r -g wildfly -d /mnt/app/wildfly -s /sbin/nologin wildfly`
...
When the download is completed, unpack the tar.gz file and move it to the /mnt/app directory:

`tar xf /tmp/wildfly-$WILDFLY_VERSION.tar.gz -C /mnt/app`

Next, create a symbolic link wildfly that will point to the WildFly installation directory:

`ln -s /mnt/app/wildfly-$WILDFLY_VERSION /mnt/app/wildfly`


WildFly will run under the wildfly user which needs to have access to the WildFly installation directory.

`chown -RH wildfly: /mnt/app/wildfly`
...
`mkdir -p /etc/wildfly`
`cp /mnt/app/wildfly/docs/contrib/scripts/systemd/wildfly.conf /etc/wildfly/`


Next copy the WildFly launch.sh script to the /opt/wildfly/bin/ directory:

`cp /mnt/app/wildfly/docs/contrib/scripts/systemd/launch.sh /mnt/app/wildfly/bin/`

edit the script: 

`vi /mnt/app/wildfly/bin/launch.sh`

![image.png](/.attachments/image-2b7e317e-dfff-4de6-ad3c-227ce2477c8e.png)


The scripts inside bin directory must have executable flag:

sh -c 'chmod +x /mnt/app/wildfly/bin/*.sh'

## Configure the standalone file

`vi /mnt/app/wildfly-10.0.0.Final/standalone/configuration/standalone.xml`
...
<server xmlns="urn:jboss:domain:4.0">
    ...
    <socket-binding-group name="standard-sockets" default-interface="public" port-offset="${jboss.socket.binding.port-offset:0}">
        ...
        <socket-binding name="http" port="${jboss.http.port:9900}"/>
...
`cp /mnt/app/wildfly/docs/contrib/scripts/systemd/wildfly.service /etc/systemd/system/`


Edit this file : 
vi /etc/systemd/system/wildfly.service

![image.png](/.attachments/image-865c2272-8e2a-4fb1-99b7-1628c0e5bd6b.png)
